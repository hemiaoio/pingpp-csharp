...
namespace Pingpp.Models {
    public class Charge: Pingpp {
        ...
        public static async Task<Charge> Create(Dictionary<string, object> chParams) {
            var ch = await Requestor.DoRequest(BaseUrl, "POST", chParams);
        ...
        public static async Task<Charge> Retrieve(string id) {
            ...
            var ch = await Requestor.DoRequest(url, "GET");
            return Mapper<Charge>.MapFromJson(ch);
        }

        public static async Task<ChargeList> List(Dictionary<string, object> listParams = null) {
