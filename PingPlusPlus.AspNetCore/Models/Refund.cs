...
namespace Pingpp.Models
{
    public class Refund : Pingpp
    {
        ...
        public static async Task<Refund> Create(string id, Dictionary<string, object> reParams)
        {
            var url = string.Format("{0}/{1}/refunds", BaseUrl, id);
            var re = await Requestor.DoRequest(url, "POST", reParams);
            return Mapper<Refund>.MapFromJson(re);
        }

        public static async Task<Refund> Retrieve(string chId, string reId)
        {
            var url = string.Format("/v1/charges/{0}/refunds/{1}", chId, reId);
            var re = await Requestor.DoRequest(url, "Get");
            return Mapper<Refund>.MapFromJson(re);
        }
